...
def list_company_policies(db: Session = Depends(get_db)):
...
def create_company_policy(payload: CompanyPolicyCreate, db: Session = Depends(get_db)):
...
def expire_company_policy(policy_id: int, db: Session = Depends(get_db)):
    """구버전 정책을 비활성화 처리."""
...
def list_refund_policies(db: Session = Depends(get_db)):
...
def create_refund_policy(payload: RefundPolicyCreate, db: Session = Depends(get_db)):
...
def expire_refund_policy(refund_policy_id: int, db: Session = Depends(get_db)):
    policy = (
